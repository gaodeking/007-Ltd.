...
### v0.2.1 (Current)
**主题：股市经济平衡系统 + BIGINT 扩容 + 阶梯税费**

#### 本次会话完成的改动

**1. 股市经济平衡与防刷机制**
- **非对称波动算法**：修改 `server/routes/stocks.js` 的 `updateStockPrice` 函数。
  - 上涨时波动率减半 (`volatility * 0.5`)，下跌时波动率翻倍 (`volatility * 2.0`)。
  - 抑制无限上涨趋势，加速泡沫破裂后的均值回归。
- **价格天花板 (2x Center Price)**：
  - 新增 `PRICE_CEILING_RATIO = 2.0` 常量，限制股价最高为中心价的 200%。
  - 触顶后允许短暂震荡，但强制阻止进一步上涨，随后触发更强均值回归拉力。
  - 买入接口增加触顶检测，显示"股价已触顶，请稍后买入"。
- **阶梯盈利税费**：替换原有的固定 2% 手续费。
  - 利润 < 10,000：税率 2%
  - 10,000 ≤ 利润 ≤ 100,000：税率 5%
  - 利润 > 100,000：税率 10%
  - 卖出时自动计算并扣除，前端显示扣税提示。
- **持仓上限 (500 股)**：
  - 单只股票每玩家最多持有 500 股。
  - 买入前校验剩余容量，前端显示 `持有量 / 500` 进度。
  - 超限或触顶时禁用买入按钮并弹窗警告。

**2. 数据库 BIGINT 扩容**
- **迁移脚本**：新建 `server/migrations/018_upgrade_to_bigint.sql`。
  - `players.money` 从 `INTEGER` 升级为 `BIGINT`（解决 21 亿溢出崩溃）。
  - `player_stocks.avg_cost` 从 `INTEGER` 升级为 `BIGINT`。
- **部署自动执行**：Render 启动时自动运行迁移，确保生产库结构同步。

**3. 10 万金币门槛绕过**
- **前端逻辑**：修改 `StockEntryModal.jsx` 和 `ArcadePanel.jsx`。
  - 新增 `holdings` 状态，进入入口前查询玩家持仓。
  - 若玩家持有任意股票 (`holdings.length > 0`)，则忽略 10 万金币门槛，允许直接返回股市。
  - 入口提示文案根据持有状态动态切换（"欢迎回来" vs "风险提示"）。

**4. 前端 UI 优化**
- **StockModal.jsx**：资产栏显示阶梯税率提示和持仓限仓说明。
- **StockCard.jsx**：
  - 显示 `持有量 / 500` 容量指示器。
  - 触顶时显示橙色警告 "🚫 股价触顶，暂停买入"。
  - 买入按钮在触顶/限仓/熔断状态下自动禁用。
- **交易反馈**：卖出成功后若产生税费，弹窗提示已扣除金额。

---

### v0.2.0 (Current)
**主题：数据库迁移系统 + 幽灵座位修复 + 十连抽优化**

#### 本次会话完成的改动

**1. 数据库迁移系统 (Database Migration)**
- **引入工具**：集成 `node-pg-migrate`，实现数据库结构变更的自动化管理。
- **自动化部署**：修改 `render.yaml`，在服务器启动前自动执行 `npm run migrate`，确保生产/测试环境数据库与代码同步。
- **同步脚本**：新建 `server/migrations/001_sync_main_to_develop.sql`，补全 `main` 分支领先的所有数据库变更（表、字段、索引）。
- **工作流更新**：更新 `AGENTS.md`，强制要求所有数据库变更必须通过迁移脚本提交，禁止手动操作 Supabase 控制台。

**2. 幽灵座位问题修复**
- **后端事务**：重构 `server/routes/seats.js` 的入座逻辑，使用数据库事务 (`BEGIN`/`COMMIT`) 和行锁 (`FOR UPDATE`)，确保“释放旧座位”和“设置新座位”的原子性。
- **唯一索引**：在 `seats` 表添加 `playerId` 的部分唯一索引，从数据库层面防止同一玩家占用多个座位。
- **脏数据清理**：提供 SQL 脚本清理历史脏数据，确保索引创建成功。

**3. 十连抽“再来一次”卡流程修复**
- **状态重置**：修改 `GachaModal.jsx`，在发起新抽奖前强制关闭旧结果弹窗，并使用 `setTimeout` 确保 React 组件完全卸载/挂载，触发动画重播。
- **防御性编程**：在 `GachaResultModal.jsx` 中增加金币不足检测，自动禁用“再来一次”按钮并显示提示，消除静默失败。
- **错误反馈**：增加抽奖失败时的 `alert` 提示，提升用户体验。

**4. 十连抽数量不足修复**
- **后端逻辑**：修复 `server/routes/gacha.js` 中当库存/上限耗尽时提前 `break` 的问题，改为自动填充“谢谢参与”，确保十连抽始终返回 10 个结果。
